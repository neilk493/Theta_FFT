...
Pipeline Structure
The pipeline is fully modular. Each stage is a separate script. Analysis logic, plotting logic, and preprocessing logic are never mixed. Every module writes its outputs as CSV or JSON files that serve as the inputs to the next stage.
...
Module 4: Validation Against Known Structure Labels (IN PROGRESS)
Script: validation.py
Input: output/comparison/, pdb/
Output: output/validation/
Maps DSSP secondary structure assignments from PDB files onto the window-level feature table by residue position. Evaluates motif cluster purity against DSSP labels. Computes precision and recall for each structural class. In progress.
Plotting Scripts
...
plot_comparison.py reads all outputs from output/comparison/ and produces five publication-quality figures saved to output/comparison_plots/ including a t-SNE embedding of all windows colored by protein and by motif cluster, a protein similarity heatmap with structural class banding, a spectral distance distribution plot by structural class pairing, and a motif cluster protein composition chart
Directory Structure
Theta_FFT/
...
The t-SNE embedding of 1123 windows from 9 proteins shows clear spatial separation of helix-dominated windows from sheet and mixed windows with no structural labels used during computation. The two helix proteins form a compact isolated cluster in the embedding.
...
pandas
matplotlib
All analysis modules use only Python standard library, numpy, and pandas. No scipy, no sklearn, no external FFT libraries. The t-SNE implementation in plot_comparison.py is written from scratch in numpy.
How to Run
